...
const Line = {
    create: async(data) => {
        const query = 'INSERT INTO lines (name, worktime, workdays, type, details, logo) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *'
        const values = [data.name, data.worktime, data.workdays, data.type, data.details, data.logo]
    ...
    update: async (id, data) => {
        ...
        const query = 'UPDATE lines SET name = $1, worktime = $2, workdays = $3, type = $4, details = $5, logo = $6 WHERE id_line = $7 RETURNING *'
        const values = [data.name, data.worktime, data.workdays, data.type, data.details, data.logo, id]
        const { rows } = await pool.query(query, values)
        return rows[0]
    },
    ...
    delete: async (id) => {
        ...
        if (parseInt(drivers_count) > 0 || parseInt(units_count) > 0 || parseInt(routes_count) > 0) {
            console.log(`Fallo al eliminar linea ${id}: conductores=${drivers_count}, unidades=${units_count}, rutas=${routes_count}`);
